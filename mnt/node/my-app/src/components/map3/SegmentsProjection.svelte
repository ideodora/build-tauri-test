<script lang="ts">
	...
	import { endIcon, key, L, startIcon, type MapContext } from '~/components/map3/leaflet';
	...
	$: if ($segmentStoreArray) {
		...
		for (const segment of $segmentStoreArray) {
			if (segment.properties.kind === 'SegmentStart') {
				segment.properties.icon = startIcon();
			}
			if (segment.properties.kind === 'SegmentEnd') {
				segment.properties.icon = endIcon();
			}

			if (
				segment.properties.kind === 'SegmentLine' ||
				segment.properties.kind === 'SegmentStart' ||
				segment.properties.kind === 'SegmentEnd'
			) {
